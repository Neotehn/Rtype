...
class EntityViewer {
 public:
   ...
   public:
    ...
    bool operator==(const Iterator& other) const {
      return index == other.index ||
             index == entityManagerI->getEntities().size();
    }
    ...
    bool ValidIndex() {
      bool validEntityID =
          IsEntityValid(entityManagerI->getEntities()[index].id);
      bool correctComponentMask =
          (all || mask == (mask & entityManagerI->getEntities()[index].mask));
  ...
  const Iterator begin() const {
    ...
    while (firstIndex < entityManager->getEntities().size() &&
           (componentMask != (componentMask &
                              entityManager->getEntities()[firstIndex].mask) ||
  ...
  const Iterator end() const {
    return Iterator(entityManager,
                    EntityIndex(entityManager->getEntities().size()),
                    componentMask, all);
  }
