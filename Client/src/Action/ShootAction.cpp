...
ShootAction::ShootAction(EntityID t_id, int t_damage, int t_type) : IAction
(ActionType::SHOOT, t_id) {
  m_damage = t_damage;
  m_type = t_type;
}
...
int ShootAction::getDamage() const {
  return m_damage;
}

int ShootAction::getShootType() const {
  return m_type;
}
