    ...
    return std::make_shared<Action>(
      CreateAction(id, CreateAction::PLAYER, rtype::Vector2f{x, y}, sprite_path,
                   ...
                   return std::make_shared<Action>(CreateAction(
                     id, CreateAction::ENEMY, rtype::Vector2f{x, y}, "",
                     action_id, velocity));
                   ...
                   return std::make_shared<Action>(CreateAction(
                     id, CreateAction::BULLET, rtype::Vector2f{x, y}, owner_id,
                     damage, shooting_type));
  } else if (type == Action::ObjectType::EXPLOSION) {
                   return std::make_shared<Action>(CreateAction(
                     id, CreateAction::EXPLOSION, rtype::Vector2f{x, y}, "",
                     action_id, velocity));
  } else if (type == Action::ObjectType::POWER_UP) {
                   return std::make_shared<Action>(CreateAction(
                     id, CreateAction::POWER_UP, rtype::Vector2f{x, y}, "",
                     action_id, velocity));
  } else if (type == Action::ObjectType::ITEM) {
                   velocity = std::stof(commands[6]);
                   return std::make_shared<Action>(
                     CreateAction(id, CreateAction::ITEM, rtype::Vector2f{x, y},
                                  "", action_id, velocity));
  } else {
...
IProtocol::getIncreaseAction(std::vector<std::string> commands, int action_id,
  ...
  if (type == Action::IncreaseType::SPEED) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::SPEED, value, action_id));
  } else if (type == Action::IncreaseType::FIRE_SHOT) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::FIRE_SHOT, value, action_id));
  } else if (type == Action::IncreaseType::DAMAGE_I) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::DAMAGE_I, value, action_id));
  } else if (type == Action::IncreaseType::LIFE) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::LIFE, value, action_id));
  } else if (type == Action::IncreaseType::SHIELD) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::SHIELD, value, action_id));
  } else if (type == Action::IncreaseType::BOMB_SHOT) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::BOMB_SHOT, value, action_id));
  } else if (type == Action::IncreaseType::COINS) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::COINS, value, action_id));
  } else if (type == Action::IncreaseType::KILLS) {
                   return std::make_shared<Action>(IncreaseAction(
                     id, IncreaseAction::KILLS, value, action_id));
  } else {
                   return std::make_shared<Action>(VoidAction(id, 0));
  }
}
...
std::shared_ptr<Action> IProtocol::getAction(std::string command) {
  ...
  if (action_type == "START") {
                   ...
                   return std::make_shared<Action>(StateAction(
                     Action::ActionType::START, id, action_id, player_id));
  } else if (action_type == "RESTART") {
                   return std::make_shared<Action>(
                     StateAction(Action::ActionType::RESTART, id, action_id));
  } else if (action_type == "UP") {
                   return std::make_shared<Action>(MovementAction(
                     Action::ActionType::UP, id, false, action_id));
  } else if (action_type == "DOWN") {
                   return std::make_shared<Action>(MovementAction(
                     Action::ActionType::DOWN, id, false, action_id));
  } else if (action_type == "LEFT") {
                   return std::make_shared<Action>(MovementAction(
                     Action::ActionType::LEFT, id, false, action_id));
  } else if (action_type == "RIGHT") {
                   return std::make_shared<Action>(MovementAction(
                     Action::ActionType::RIGHT, id, false, action_id));
  } else if (action_type == "POS") {
                   rtype::Vector2f pos = {std::stof(commands[3]),
                                          std::stof(commands[4])};
                   return std::make_shared<Action>(
                     PosAction(id, pos, action_id));
  } else if (action_type == "SHOOT") {
                   int damage = std::stoi(commands[4]);
                   Action::ShootingType type =
                     Action::ShootingType(std::stoi(commands[5]));
                   return std::make_shared<Action>(
