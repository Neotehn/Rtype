...
void UdpServer::startListening() {
  ...
  m_socket.async_receive_from(boost::asio::buffer(session->m_recvBuffer),
...
void UdpServer::handleListening(shared_session t_session,
  ...
  boost::asio::post(m_socket.get_executor(),
                    boost::bind(&UdpSession::handleRequest,
                                t_session,
                                t_ec,
                                t_bytesTransferred));
...
void UdpServer::enqueueResponseStrand(shared_session const& t_session) {
  m_socket.async_send_to(boost::asio::buffer(t_session->m_message),
                         t_session->m_remoteEndpoint,
                         m_strand.wrap(
                            boost::bind(&UdpSession::handleSent,
                            t_session,
                            boost::asio::placeholders::error,
                            boost::asio::placeholders::bytes_transferred)));
}
